...
#define PROID			"sjdcGGYyKs"						//产品ID

#define ACCESS_KEY		"WFp4REg1dTZESzZDUkNKNnhwSllZeUwydXdEMTFQR0Y="		//设备密钥
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Status\":{\"value\":%d},", Status);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"RespiratoryRate\":{\"value\":%d},", RespiratoryRate);
	strcat(buf, text);
